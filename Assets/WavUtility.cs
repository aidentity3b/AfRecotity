...
public class WavUtility
{
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings", double time = -1)
    {
        ...
        //Add: Determine Length to save
        int samplesToSave;
        if(time == -1)
        {
            //Save full clip
            samplesToSave = audioClip.samples;
        }
        else
        {
            samplesToSave = Mathf.FloorToInt(((float)time / audioClip.length) * audioClip.samples);
        }


        // total file size = 44 bytes for header format and audioClip.samples * factor due to float to Int16 / sbyte conversion
        //int fileSize = audioClip.samples * BlockSize_16Bit + headerSize; // BlockSize (bitDepth)
        int fileSize = samplesToSave * BlockSize_16Bit + headerSize;
        ...
        // data chunks (data)
        WriteFileData(ref stream, audioClip, bitDepth, samplesToSave);
    ...
    private static int WriteFileData(ref MemoryStream stream, AudioClip audioClip, UInt16 bitDepth, int samplesToSave)
    {
        int count = 0;
        int total = 8;
        ...
        float[] data = new float[samplesToSave * audioClip.channels];
        Array.Copy(rawData, data, samplesToSave * audioClip.channels);
        ...
        int subchunk2Size = Convert.ToInt32(samplesToSave * BlockSize_16Bit); // BlockSize (bitDepth)
